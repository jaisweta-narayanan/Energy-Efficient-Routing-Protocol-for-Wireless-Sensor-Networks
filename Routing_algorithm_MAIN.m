...
By = sink.y;
  plot(Bx,By,'k*')
   hold on
beacon_length = 20;%jj2000
ctl_length= 20;%jj300
data_length=4000;%
En = 0.6; %Initial energy of all nodes
...
for ii=2:25   
        ...
        plot([S3(connect_node4(ii-1)).xd S3(connect_node4(ii)).xd],[S3(connect_node4(ii-1)).yd S3(connect_node4(ii)).yd],'-ko');
        hold on;
...
not_in_tree1 = not_in_tree1(:);  %not_in_tree holds the set of nodes not yet included in the tree
  ...
  for ii = 1: N1
      ...
      disp(S3(not_in_tree1(ii)).index_A); %jj tree1
      fprintf('\n');
...
count2 = 0;
%if STATISTICS3(r+1).ALLIVE2~= 0 jj1-2
for ii= 1: 25
      if (S3(connect_node2(ii)).E > 0)
...
not_in_tree2 = not_in_tree2(:);  %not_in_tree holds the set of nodes not yet included in the tree
  ...
  for ii = 1: N1
      ...
      disp(S3(not_in_tree2(ii)).index_B);%jj tree2 a2b
      fprintf('\n');
...
count3 = 0;
%if STATISTICS3(r+1).ALLIVE3~= 0 jj1-3
for ii= 1: 25
      if (S3(connect_node3(ii)).E > 0)
...
not_in_tree3 = not_in_tree3(:);  %not_in_tree holds the set of nodes not yet included in the tree
  ...
  for ii = 1: N1
      ...
      disp(S3(not_in_tree3(ii)).index_C);%jj
      fprintf('\n');
...
count4 = 0;
%if STATISTICS3(r+1).ALLIVE4~= 0 jj1-4
for ii= 1: 25
      if (S3(connect_node4(ii)).E > 0)
...
not_in_tree4 = not_in_tree4(:);  %not_in_tree holds the set of nodes not yet included in the tree
  ...
  for ii = 1: N1
      ...
      disp(S3(not_in_tree4(ii)).index_D);%jj
      fprintf('\n');
